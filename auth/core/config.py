from pathlib import Path
from typing import Annotated

from pydantic import PostgresDsn, SecretStr, field_validator
from pydantic_core.core_schema import ValidationInfo
from pydantic_settings import BaseSettings, SettingsConfigDict

BASEDIR = Path(__file__).resolve().parent.parent.parent
APP_DIR = BASEDIR / "auth"


class Settings(BaseSettings):
    # App
    APP_TITLE: str = "Async architecture authentication service"
    APP_DESCRIPTION: str = "Default description"
    DEBUG: bool = False
    LOG_LEVEL: str = "INFO"

    # Auth
    PUBLIC_KEY: SecretStr
    PRIVATE_KEY: SecretStr
    ALGORITHM: str = "RS256"
    ACCESS_TOKEN_EXPIRE_TIME_SECONDS: int = 60 * 300

    # Postgres
    AUTH_DB_HOST: str
    AUTH_DB_USER: str
    AUTH_DB_PASSWORD: SecretStr
    AUTH_DB_NAME: str
    AUTH_DB_PORT: int
    AUTH_DB_DSN: PostgresDsn | str = ""

    # Postgres logs
    SQL_LOGS: bool = False
    SQL_POOL_LOGS: bool = False

    model_config = SettingsConfigDict(env_file=APP_DIR / ".env")

    @field_validator("AUTH_DB_DSN")
    def build_postgres_dsn(
        cls, value: PostgresDsn | None, info: ValidationInfo
    ) -> Annotated[str, PostgresDsn]:
        if not value:
            value = PostgresDsn.build(
                scheme="postgresql+asyncpg",
                username=info.data["AUTH_DB_USER"],
                password=info.data["AUTH_DB_PASSWORD"].get_secret_value(),
                host=info.data["AUTH_DB_HOST"],
                port=info.data["AUTH_DB_PORT"],
                path=f"{info.data['AUTH_DB_NAME'] or ''}",
            )
        return str(value)


app_settings = Settings()
